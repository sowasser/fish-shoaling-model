...
def run_model(boid_prior, other_prior):
    """
    ...
    model = ShoalModel(n_fish=20,
                       ...
                       speed=boid_prior,
                       vision=boid_prior,
                       separation=boid_prior,
                       cohere=other_prior,
                       separate=other_prior,
                       match=other_prior)
    ...
    all_data["speed"] = boid_prior  # add speed value column
    all_data["vision"] = boid_prior  # add vision columnm
    all_data["separation"] = boid_prior  # add separation column
    all_data["cohere"] = other_prior  # add speed value column
    all_data["separate"] = other_prior  # add vision columnm
    all_data["match"] = other_prior  # add separation column
    return pd.DataFrame(all_data).T


# Run model with prior called in create_tasks.py, as a float
model_data = run_model(float(sys.argv[1]), float(sys.argv[2]))
