"""
Modifying the ContinuousSpace class in Mesa to work in 3D.
...
def accept_tuple_argument(wrapped_function):
    """ Decorator to allow grid methods that take a list of (x, y, z) position tuples
...
class Grid:
    ...
    Properties:
        width, height: The grid's width, height, and depth.
    ...
    """
    def __init__(self, width, height, depth, torus):
        """ Create a new grid.
    ...
    def coord_iter(self):
        ...
        for row in range(self.width):
            for col in range(self.height):
                for col2 in range(self.depth):
                    yield self.grid[row][col][col2], row, col, col2
